 ...
 * @since 2016-04-18T14:07:00
 */
public class JavaFXVideoStageDemo {
    private static int TIMEOUT = 5000;
    private static final Logger log = LoggerFactory.getLogger(JavaFXVideoStageDemo.class);

    private static Application myApp;

    // Used to launch the application before running any test
    private static final CountDownLatch launchLatch = new CountDownLatch(1);
    private static Map<String, JavaFXVideoStage> namedWindows = new ConcurrentHashMap<>();
    ...
    public static class MyApp extends Application {
        ...
        public void init() throws Exception {
            JavaFXVideoStageDemo.myApp = this;
        }
        ...
        public void stop() throws Exception {
            JavaFXVideoStageDemo.myApp = null;
            super.stop();
        }
    }
     ...
     * @return A future that will contain a reference to the JavaFXVideoStage when the
     *         stage has finished initialization.
     *
     */
    public static CompletableFuture<JavaFXVideoStage> namedWindow(final String pathToVideo) {
        startJavaFX();
        final CompletableFuture<JavaFXVideoStage> videoStageFuture = new CompletableFuture<>();
        runOnJavaFXThread(() -> {
            JavaFXVideoStage stage;
            if (namedWindows.containsKey(pathToVideo)) {
                stage = namedWindows.get(pathToVideo);
            }
            else {
                stage = new JavaFXVideoStage(pathToVideo);
    ...
    public static void main(String[] args) throws Exception {
        ...
        Consumer<JavaFXVideoStage> fn = videoStage -> {
